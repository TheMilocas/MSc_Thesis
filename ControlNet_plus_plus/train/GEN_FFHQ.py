"""
===========================================================
ControlNet vs Base SD Inpainting on FFHQ Masked Faces
===========================================================

Purpose:
    Generate inpainted images from masked FFHQ faces using both:
      1. Base Stable Diffusion inpainting pipeline
      2. ControlNet-guided inpainting conditioned on identity embeddings

Inputs:
    - Masked FFHQ images: `IMG_DIR`
    - Eye/face mask: `MASK_DIR`
    - Identity embeddings: `EMB_DIR`
    - Pretrained ControlNet model: `CONTROLNET_PATH`
    - Base Stable Diffusion inpainting model: "runwayml/stable-diffusion-inpainting"

Outputs:
    - Inpainted images using:
        - Base SD pipeline → saved in `SAVE_DIR/base`
        - ControlNet-guided pipeline → saved in `SAVE_DIR/controlnet`
"""
...
EMB_DIR = "./DATASETS_FOR_TESTING/FFHQ/input_embeddings"
IMG_DIR = "./DATASETS_FOR_TESTING/FFHQ/input"
MASK_DIR = "./mask.png"
NUM_SAMPLES = 2953
SAVE_DIR = "./TEST_FFHQ"
...
for i in range(processed, NUM_SAMPLES):
    ...
    image_masked_path = os.path.join(IMG_DIR, name)
