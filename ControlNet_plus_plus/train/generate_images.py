"""
===========================================================
Face Image Inpainting Comparison: Base SD vs ControlNet
===========================================================

Purpose:
    Compare standard Stable Diffusion inpainting with a 
    ControlNet-guided inpainting model using masked face images 
    and identity embeddings. This allows evaluation of how 
    identity-preserving conditioning affects inpainting results.

Inputs:
    - Dataset of masked face images: specified by `DATASET_NAME` 
      (expects images and masks)
    - Identity embeddings: numpy files corresponding to each image 
      (specified by `EMBEDDING_PREFIX`)
    - Pretrained ControlNet model: `CONTROLNET_PATH`
    - Stable Diffusion base inpainting model: downloaded from 
      "runwayml/stable-diffusion-inpainting"

Outputs:
    - Inpainted images using base Stable Diffusion pipeline
      (saved in `SAVE_DIR/base`)
    - Inpainted images using ControlNet-guided pipeline
      (saved in `SAVE_DIR/controlnet`)
    - Original masked images (saved in `SAVE_DIR/input`)
"""
