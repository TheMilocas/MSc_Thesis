...
df = pd.DataFrame({
    "id": common_ids,
    "cosine_base": cosine_base,
    "cosine_controlnet": cosine_cn,
    "delta": delta
})

csv_path = os.path.join(SAVE_DIR, f"similarity_scores_{SAVE_PREFIX}.csv")
df.to_csv(csv_path, index=False)
print(f"\nSaved cosine similarities to: {csv_path}")
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, f"similarity_delta_hist_{SAVE_PREFIX}.png"), dpi=300)
...
with open(csv_path, "a") as f:
    f.write("\n# Summary Statistics\n")
    f.write(f"Mean Δ,{mean_delta:.5f}\n")
    f.write(f"% Δ > 0,{pct_gt_0:.2f}\n")
    f.write(f"% Δ > 0.01,{pct_gt_001:.2f}\n")
    f.write(f"% Δ > 0.05,{pct_gt_005:.2f}\n")
    f.write(f"% Δ < 0,{pct_lt_0:.2f}\n")
