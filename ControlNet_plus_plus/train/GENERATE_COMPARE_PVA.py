"""
===========================================================
ControlNet Face Inpainting with Masked Images
===========================================================

Purpose:
    Apply ControlNet-guided inpainting on masked face images 
    using precomputed identity embeddings to preserve identity 
    during inpainting.

Inputs:
    - Masked face dataset: specified by `DATASET_NAME_MASKS`
    - Face images: stored in `IMG_DIR`
    - Masks: stored in `MASK_DIR`
    - Identity embeddings: stored in `EMB_DIR`
    - Pretrained ControlNet model: `CONTROLNET_PATH`
    - Stable Diffusion inpainting base model: "runwayml/stable-diffusion-inpainting"

Outputs:
    - Inpainted images using ControlNet-guided pipeline, saved 
      in `SAVE_DIR` with filenames matching input image and mask.
"""
