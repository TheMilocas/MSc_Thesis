...
"""
Evaluation script for image generation experiments (PNG images).

Description:
    This script evaluates generated images produced by ControlNet and a baseline method.
    For each method:
      1. Loads generated images (.png) and matches them with test dataset samples.
...
Inputs:
    ...
    - CONTROLNET_DIR: Directory with ControlNet generated images (.png)
    - BASE_DIR: Directory with baseline generated images (.png)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for method_name, method_dir in methods.items():
        ...
        gen_paths_raw = sorted(glob.glob(os.path.join(method_dir, "*.png")))
        idx_to_gen = {}
        for p in gen_paths_raw:
            ...
            if idx is None:
                ...
                if digits:
                    idx = int(digits[-1])
            if idx is not None and 0 <= idx < n_dataset:
                idx_to_gen[idx] = p
